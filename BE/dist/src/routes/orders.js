...
router.post("/:id/checkout", authMiddleware, requireRoles("ADMIN", "MANAGER"), async (req, res) => {
    try {
        const { id } = req.params;
...
router.post("/:id/cancel", authMiddleware, requireRoles("ADMIN", "MANAGER"), async (req, res) => {
    try {
        const { id } = req.params;
        const user = req.user;
        const order = await prisma.order.findUnique({ where: { id } });
